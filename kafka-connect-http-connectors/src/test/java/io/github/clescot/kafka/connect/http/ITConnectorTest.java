...
public class ITConnectorTest {
  ...
  public static KafkaContainer kafkaContainer =
      new KafkaContainer(DockerImageName.parse("confluentinc/cp-kafka:" + CONFLUENT_VERSION))
          ...
          .withLogConsumer(
              new Slf4jLogConsumer(LOGGER).withSeparateOutputStreams().withPrefix("kafka-broker"));

  @Container
  private static final SchemaRegistryContainer schemaRegistryContainer =
      new SchemaRegistryContainer("confluentinc/cp-schema-registry:" + CONFLUENT_VERSION)
          .withNetwork(NETWORK)
  ...
  public static DebeziumContainer connectContainer =
      new DebeziumContainer("clescot/kafka-connect-http:" + VERSION)
          ...
          .withCopyFileToContainer(
              MountableFile.forClasspathResource(CLIENT_TRUSTSTORE_JKS_FILENAME),
              "/opt/" + CLIENT_TRUSTSTORE_JKS_FILENAME)
          .withExposedPorts(8083)
          //            .withExposedPorts(8083,5005)
          .withLogConsumer(
              new Slf4jLogConsumer(LOGGER).withSeparateOutputStreams().withPrefix("kafka-connect"))
  ...
  @RegisterExtension
  static WireMockExtension wmHttps =
      WireMockExtension.newInstance()
          .options(
  ...
  static void startContainers() throws IOException {
    ...
    // start containers
    Startables.deepStart(Stream.of(kafkaContainer, schemaRegistryContainer, connectContainer))
        .join();
    internalSchemaRegistryUrl =
        "http://" + schemaRegistryContainer.getNetworkAliases().get(0) + ":8081";
    externalSchemaRegistryUrl =
        "http://"
  ...
  private static void createTopics(String bootstrapServers, String... topics) {
    ...
    try (var admin = AdminClient.create(Map.of(BOOTSTRAP_SERVERS_CONFIG, bootstrapServers))) {
      ...
      admin
          .listTopics()
          .names()
          .thenApply(set -> set.stream().peek(topic -> LOGGER.info("topic:'{}' is present", topic)))
          .get();
  ...
  private static void configureSinkConnector(
    ...
    ConnectorConfiguration sinkConnectorMessagesAsStringConfiguration =
    ...
    if (PublishMode.IN_MEMORY_QUEUE.name().equalsIgnoreCase(publishMode)) {
      sinkConnectorMessagesAsStringConfiguration =
          sinkConnectorMessagesAsStringConfiguration.with("queue.name", queueNameOrProducerTopic);
    ...
    if (additionalSettings != null && additionalSettings.length > 0) {
      for (Map.Entry<String, String> additionalSetting : additionalSettings) {
        sinkConnectorMessagesAsStringConfiguration =
            sinkConnectorMessagesAsStringConfiguration.with(
                additionalSetting.getKey(), additionalSetting.getValue());
      }
    }
  ...
  private static void configureHttpSourceConnector(
      String connectorName, String queueName, String successTopic, String errorTopic) {
    // source connector
    ConnectorConfiguration sourceConnectorConfiguration =
        ConnectorConfiguration.create()
            .with(
                "connector.class",
                "io.github.clescot.kafka.connect.http.source.HttpSourceConnector")
            .with("tasks.max", "1")
  ...
  private static void configureCronSourceConnector(
      String connectorName, String topic, List<CronJobConfig> configs) {
    // source connector
    String jobs =
        Joiner.on(",")
            .join(configs.stream().map(CronJobConfig::getId).collect(Collectors.toList()));
    ConnectorConfiguration sourceConnectorConfiguration =
        ConnectorConfiguration.create()
            .with(
                "connector.class",
                "io.github.clescot.kafka.connect.http.source.cron.CronSourceConnector")
            .with("tasks.max", "1")
            .with("topic", topic)
            .with("key.converter", "org.apache.kafka.connect.storage.StringConverter")
    ...
    for (CronJobConfig config : configs) {
      ...
      if (headers != null && !headers.isEmpty()) {
        ...
        keySet.forEach(
            key ->
                sourceConnectorConfiguration.with(
                    config.getId() + ".header." + key, Joiner.on(",").join(headers.get(key))));
      }
    }
  ...
  void test_sink_and_source_with_input_as_string() throws JSONException, JsonProcessingException {
    ...
    configureHttpSourceConnector(
        "http-source-connector-test_sink_and_source_with_input_as_string",
        queueName,
        successTopic,
        errorTopic);
    configureSinkConnector(
        "http-sink-connector-test_sink_and_source_with_input_as_string",
        PublishMode.IN_MEMORY_QUEUE.name(),
        HTTP_REQUESTS_AS_STRING,
        "org.apache.kafka.connect.storage.StringConverter",
        "test_sink_and_source_with_input_as_string",
    ...
    KafkaConsumer<String, HttpExchange> consumer =
        getConsumer(kafkaContainer, externalSchemaRegistryUrl, "json");
  ...
  private static String serializeHttpExchange(ConsumerRecord<String, HttpExchange> consumerRecord) {
    ...
    return new String(
        jsonStringSerializer.serialize("dummy", consumerRecord.value()), StandardCharsets.UTF_8);
  }

  @Test
  void test_sink_in_producer_mode_with_input_as_string_and_producer_output_as_string()
    ...
    String successTopic =
        "success_test_sink_and_source_with_input_as_string_and_producer_output_as_string";
    String errorTopic =
        "error_test_sink_and_source_with_input_as_string_and_producer_output_as_string";
    ...
    ProducerRecord<String, String> record =
        new ProducerRecord<>(
            incomingTopic, null, System.currentTimeMillis(), null, httpRequestAsJSON, kafkaHeaders);
    producer.send(record);
    producer.flush();

    // verify http responses
    KafkaConsumer<String, String> consumer =
        getConsumer(kafkaContainer, externalSchemaRegistryUrl, producerOutputFormat);
  ...
  @Test
  void test_sink_in_producer_mode_with_input_as_string_and_producer_output_as_string_with_cron()
      throws JSONException, JsonProcessingException {
    ...
    CronJobConfig cronJobConfig =
        new CronJobConfig("job1", url, "0/5 * * ? * *", HttpRequest.Method.POST, "stuff", headers);
    configureCronSourceConnector(
        "cron-source-connector", incomingTopic, Lists.newArrayList(cronJobConfig));
    List<String> registeredConnectors = connectContainer.getRegisteredConnectors();
    String joinedRegisteredConnectors = Joiner.on(",").join(registeredConnectors);
    LOGGER.info("registered connectors :{}", joinedRegisteredConnectors);
  ...
  @Test
  void test_sink_in_producer_mode_with_input_as_string_and_producer_output_as_json()
      throws JSONException, JsonProcessingException {
    ...
    String successTopic =
        "success_test_sink_and_source_with_input_as_string_and_producer_output_as_json";
    String errorTopic =
        "error_test_sink_and_source_with_input_as_string_and_producer_output_as_json";
    ...
    String expectedJSON =
        "{\n"
            ...
            + "}";
    Serializer jsonStringSerializer = new KafkaJsonSerializer();
    String httpExchangeAsString =
        new String(jsonStringSerializer.serialize("dummy", httpExchange), StandardCharsets.UTF_8);
    JSONAssert.assertEquals(
        expectedJSON,
        httpExchangeAsString,
        new CustomComparator(
  ...
  private static void checkConnectorPluginIsInstalled() {
    ...
    Request request = builder.url(connectContainer.getTarget() + "/connector-plugins").build();
    Awaitility.await()
  ...
  void test_sink_and_source_with_input_as_struct_and_schema_registry() throws JSONException {
    ...
    configureHttpSourceConnector(
        "http-source-connector-test_" + suffix, "test_" + suffix, successTopic, errorTopic);
    configureSinkConnector(
        "http-sink-connector-test_" + suffix,
        PUBLISH_TO_IN_MEMORY_QUEUE_OK,
        incomingTopic,
        "io.confluent.connect.json.JsonSchemaConverter",
        "test_" + suffix,
        new AbstractMap.SimpleImmutableEntry<>(
            "value.converter.schema.registry.url", internalSchemaRegistryUrl),
        new AbstractMap.SimpleImmutableEntry<>(CONFIG_GENERATE_MISSING_REQUEST_ID, "true"),
        new AbstractMap.SimpleImmutableEntry<>(CONFIG_GENERATE_MISSING_CORRELATION_ID, "true"));
    List<String> registeredConnectors = connectContainer.getRegisteredConnectors();
    String joinedRegisteredConnectors = Joiner.on(",").join(registeredConnectors);
    ...
    httpRequest.setBodyAsString("stuff");
    Collection<Header> kafkaHeaders = Lists.newArrayList();
    ProducerRecord<String, HttpRequest> record =
        new ProducerRecord<>(
            incomingTopic, null, System.currentTimeMillis(), null, httpRequest, kafkaHeaders);
    producer.send(record);
    producer.flush();
  ...
  void test_retry_policy() throws JSONException {
    ...
    configureSinkConnector(
        ...
        new AbstractMap.SimpleImmutableEntry<>(
            CONFIG_DEFAULT_RETRY_RESPONSE_CODE_REGEX, DEFAULT_DEFAULT_RETRY_RESPONSE_CODE_REGEX));
    List<String> registeredConnectors = connectContainer.getRegisteredConnectors();
    String joinedRegisteredConnectors = Joiner.on(",").join(registeredConnectors);
    LOGGER.info("registered connectors :{}", joinedRegisteredConnectors);
    ...
    wireMock.register(
        WireMock.post(WireMock.urlEqualTo("/ping"))
  ...
  @Test
  void test_sink_and_source_with_input_as_struct_and_schema_registry_test_rate_limiting()
    ...
    configureSinkConnector(
        ...
        new AbstractMap.SimpleImmutableEntry<>(
            CONFIG_DEFAULT_RATE_LIMITER_MAX_EXECUTIONS, maxExecutionsPerSecond + ""));
    List<String> registeredConnectors = connectContainer.getRegisteredConnectors();
    String joinedRegisteredConnectors = Joiner.on(",").join(registeredConnectors);
    LOGGER.info("registered connectors :{}", joinedRegisteredConnectors);
    ...
    for (int i = 0; i < messageCount; i++) {
      ConsumerRecord<String, HttpExchange> consumerRecord = consumerRecords.get(i);
      checkMessage(
          successTopic, escapedJsonResponse, 200, statusMessage, baseUrl, consumerRecord, 1);
      checkedMessages++;
    }
    stopwatch.stop();
  ...
  void test_custom_truststore() throws JSONException {
    ...
    String queueName = "test_" + suffix;
    configureHttpSourceConnector(
        "http-source-connector-test_" + suffix, queueName, successTopic, errorTopic);
    configureSinkConnector(
        "http-sink-connector-test_" + suffix,
        PUBLISH_TO_IN_MEMORY_QUEUE_OK,
        incomingTopic,
        ...
        new AbstractMap.SimpleImmutableEntry<>(CONFIG_GENERATE_MISSING_CORRELATION_ID, "true"),
        new AbstractMap.SimpleImmutableEntry<>(
            CONFIG_DEFAULT_OKHTTP_SSL_SKIP_HOSTNAME_VERIFICATION, "true"),
        new AbstractMap.SimpleImmutableEntry<>(CONFIG_DEFAULT_OKHTTP_PROTOCOLS, "HTTP_1_1"),
        new AbstractMap.SimpleImmutableEntry<>(
            CONFIG_HTTP_CLIENT_SSL_TRUSTSTORE_PATH, "/opt/" + CLIENT_TRUSTSTORE_JKS_FILENAME),
        new AbstractMap.SimpleImmutableEntry<>(
            CONFIG_HTTP_CLIENT_SSL_TRUSTSTORE_PASSWORD, CLIENT_TRUSTSTORE_JKS_PASSWORD),
        new AbstractMap.SimpleImmutableEntry<>(
            CONFIG_HTTP_CLIENT_SSL_TRUSTSTORE_TYPE, JKS_STORE_TYPE),
        new AbstractMap.SimpleImmutableEntry<>(
            CONFIG_HTTP_CLIENT_SSL_TRUSTSTORE_ALGORITHM, TRUSTSTORE_PKIX_ALGORITHM));
    List<String> registeredConnectors = connectContainer.getRegisteredConnectors();
    String joinedRegisteredConnectors = Joiner.on(",").join(registeredConnectors);
    LOGGER.info("registered connectors :{}", joinedRegisteredConnectors);
  ...
  private static void checkMessage(
      String topicName,
      String escapedJsonResponse,
      int statusCode,
      String statusMessage,
      String baseUrl,
      ConsumerRecord<String, HttpExchange> consumerRecord,
      int attempts)
      throws JSONException {
  ...
  private KafkaProducer<String, String> getStringProducer(KafkaContainer kafkaContainer) {
  ...
  private <T> KafkaProducer<String, T> getStructAsJSONProducer() {
    ...
    props.put(
        ...
        SpecificationVersion.DRAFT_2019_09.toString());
  ...
  private <T> KafkaConsumer<String, T> getConsumer(
      KafkaContainer kafkaContainer, String schemaRegistryUrl, String format) {

    SchemaRegistryClient schemaRegistryClient =
        new CachedSchemaRegistryClient(
            schemaRegistryUrl,
            CACHE_CAPACITY,
            Lists.newArrayList(new JsonSchemaProvider(), new AvroSchemaProvider()),
            Maps.newHashMap());
    ...
    switch (format) {
      case "json":
        ...
        deserializer =
            new KafkaJsonSchemaDeserializer<>(schemaRegistryClient, props, HttpExchange.class);
        break;
      case "string":
      default:
  ...
  private <T> List<ConsumerRecord<String, T>> drain(
      KafkaConsumer<String, T> consumer, int expectedRecordCount, int timeoutInSeconds) {
