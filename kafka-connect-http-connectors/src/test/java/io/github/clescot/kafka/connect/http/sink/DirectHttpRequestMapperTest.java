...
import static io.github.clescot.kafka.connect.http.HttpRequestAdapter.SCHEMA;
...
class DirectHttpRequestMapperTest {
  ...
  class TestMap {
    ...
    void test_buildHttpRequest_http_request_as_json_schema() {
      ...
      byte[] httpRequestAsJsonSchemaWithConverter =
          jsonSchemaConverter.fromConnectData(
              topic,
              SCHEMA,
              HttpRequestAdapter.from((HttpRequest) dummyHttpRequest.clone()).toStruct());

      SchemaAndValue schemaAndValue =
          jsonSchemaConverter.toConnectData(topic, httpRequestAsJsonSchemaWithConverter);
    ...
    void test_buildHttpRequest_http_request_as_struct() {
      ...
      SinkRecord sinkRecord =
          new SinkRecord(
              ...
              "key",
              Schema.STRING_SCHEMA,
              getDummyHttpRequestAsStruct(DUMMY_URL),
              -1,
              System.currentTimeMillis(),
              TimestampType.CREATE_TIME,
              headers);
  ...
  @NotNull
  private static JsonSchemaConverter getJsonSchemaConverter(
  ...
  private Struct getDummyHttpRequestAsStruct(String url) {
    HttpRequestAdapter httpRequest = HttpRequestAdapter.from(getDummyHttpRequest(url));
