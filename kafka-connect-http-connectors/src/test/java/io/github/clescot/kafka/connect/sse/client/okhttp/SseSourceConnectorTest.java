  ...
  class TaskConfigs {
    ...
    void test_0_tasks() {
      Assertions.assertThrows(
          IllegalArgumentException.class, () -> sseSourceConnector.taskConfigs(0));
    }

    @Test
    void test_negative_tasks() {
      Assertions.assertThrows(
          IllegalArgumentException.class, () -> sseSourceConnector.taskConfigs(-1));
    }
    ...
    void test_two_sse_endpoints_with_one_task() {
      ...
      assertThat(taskConfigs.get(0))
          .containsEntry("config.default.url", "http://localhost:8080/sse");
      assertThat(taskConfigs.get(0)).containsEntry("config.default.topic", "test");
      assertThat(taskConfigs.get(0))
          .containsEntry("config.second.url", "http://localhost:8080/sse2");
    ...
    void test_two_sse_endpoints_with_two_task() {
      ...
      assertThat(taskConfigs.get(0)).containsEntry("config.default.topic", "test");
      assertThat(taskConfigs.get(0))
          .doesNotContainEntry("config.second.url", "http://localhost:8080/sse2");
      ...
      assertThat(taskConfigs.get(1))
          .containsEntry("config.second.url", "http://localhost:8080/sse2");
      assertThat(taskConfigs.get(1)).containsEntry("config.second.topic", "test2");
      assertThat(taskConfigs.get(1))
          .doesNotContainEntry("config.default.url", "http://localhost:8080/sse");
