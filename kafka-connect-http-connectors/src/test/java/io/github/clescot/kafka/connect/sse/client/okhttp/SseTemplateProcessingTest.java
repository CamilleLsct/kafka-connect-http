...
class SseTemplateProcessingTest {
    ...
    class TemplateProcessing {
        ...
        void test_poll_with_template_processing() {
            ...
            // Add template configuration that extracts the message using correct JSONPath syntax
            ...
            // The data should be the full SSE event JSON (SSE connector always returns complete event structure)
            assertThat(record.value()).isEqualTo("{\"id\":\"test-id\",\"attributes\":\"{}\",\"type\":\"test-type\",\"data\":\"{\"message\": \"hello\"}\"}");
        }
    }
}