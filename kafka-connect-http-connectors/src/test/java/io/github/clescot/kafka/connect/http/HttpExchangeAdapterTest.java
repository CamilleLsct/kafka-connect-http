...
class HttpExchangeAdapterTest {
  ...
  void setup() throws RestClientException, IOException {
    ...
    jsonSchemaDeserializerConfig.put(
        KafkaJsonSchemaDeserializerConfig.JSON_VALUE_TYPE, HttpExchange.class.getName());
    ...
    deserializer =
        new KafkaJsonSchemaDeserializer<>(
            schemaRegistryClient, jsonSchemaDeserializerConfig, HttpExchange.class);
  }
  ...
  class TestToStruct {
    ...
    void test_to_struct() {
      ...
      HttpExchange httpExchange =
          new HttpExchange(
              getDummyHttpRequest(), getDummyHttpResponse(200), 100, moment, attempts, SUCCESS);
      ...
      assertThat(httpExchangeAdapter.toStruct().get("moment"))
          .isEqualTo(moment.format(DateTimeFormatter.ISO_ZONED_DATE_TIME));
  ...
  class TestSerialization {
    ...
    void test_serialize_http_exchange() {
      ...
      AssertionsForClassTypes.assertThat(deserializedHttpExchange.getRequest())
          .isEqualTo(httpExchange.getRequest());
      AssertionsForClassTypes.assertThat(deserializedHttpExchange.getResponse())
          .isEqualTo(httpExchange.getResponse());
  ...
  private HttpRequest getDummyHttpRequest() {
    HttpRequest httpRequest = new HttpRequest("http://www.toto.com", HttpRequest.Method.GET);
