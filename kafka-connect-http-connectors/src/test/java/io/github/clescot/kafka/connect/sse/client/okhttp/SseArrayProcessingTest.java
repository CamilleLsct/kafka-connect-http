...
class SseArrayProcessingTest {
    ...
    @Test
    void test_array_processing_with_jsonpath() {
        ...
        // Template to extract all item IDs from array
        ...
        assertThat(processedEvent.getData()).isEqualTo("{\"items\": [{\"id\": 1, \"name\": \"Item1\"}, {\"id\": 2, \"name\": \"Item2\"}, {\"id\": 3, \"name\": \"Item3\"}]}");
    }
    ...
    void test_array_processing_with_filtering() {
        ...
        // Template to extract only active products
        settings.put("config."+configurationId+".exchange.template", "${jsonpath:$.content}");
        settings.put("config."+configurationId+".exchange.template.processors", "jsonpath");
