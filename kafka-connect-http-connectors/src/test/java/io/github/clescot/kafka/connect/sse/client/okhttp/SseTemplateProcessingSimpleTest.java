  ...
  class TemplateProcessing {
    ...
    void test_template_processing_with_jsonpath_processor() {
      ...
      SseConfiguration sseConfiguration =
          new SseConfiguration(configurationId, mockHttpClient, settings);

      // Process the event using the template
      HttpRequest httpRequest = new HttpRequest("http://example.com/sse");
      SseEvent processedEvent =
          sseConfiguration.processEventWithTemplate(originalEvent, httpRequest);
    ...
    void test_template_processing_with_jsonpath_message_extraction() {
      // Create a simple SSE event
      SseEvent originalEvent =
          new SseEvent("test-id", "test-type", "{\"message\": \"hello\", \"timestamp\": 12345}");
      ...
      assertThat(processedEvent).isNotNull();
      assertThat(processedEvent.getData())
          .isEqualTo("{\"message\": \"hello\", \"timestamp\": 12345}");
      // The template processing should have added jsonpath attributes to the exchange
      // We can't easily verify this from the SseEvent, but we can verify the template manager
      // worked
    }
  }
}
