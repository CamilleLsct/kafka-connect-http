...
public class HttpCronSourceTask extends SourceTask {
  ...
  public void start(Map<String, String> settings) {
    ...
    try {
      ...
      scheduler.start();
      String jobIds =
          Optional.ofNullable(settings.get(HttpCronSourceConfigDefinition.JOBS))
              .orElseThrow(() -> new IllegalArgumentException("jobs configuration is required"));
      ...
      jobs.forEach(
          jobId -> {
            ...
            Optional<String> methodAsString =
                Optional.ofNullable(settings.get(JOB_PREFIX + jobId + ".method"));
            methodAsString.ifPresent(method -> jobDataMap.put(METHOD, method));

            Optional<String> bodyAsString =
                Optional.ofNullable(settings.get(JOB_PREFIX + jobId + ".body"));
            bodyAsString.ifPresent(body -> jobDataMap.put(BODY, body));

            Optional<String> headersAsString =
                Optional.ofNullable(settings.get(JOB_PREFIX + jobId + ".headers"));
            ...
            if (headersAsString.isPresent()) {
              ...
              headerKeys.forEach(
                  key -> jobDataMap.put(key, settings.get(JOB_PREFIX + jobId + ".header." + key)));
              jobDataMap.put(HEADERS, headersAsString.get());
            }

            JobDetail job =
                newJob(HttpCronJob.class).withIdentity(jobId).setJobData(jobDataMap).build();
