...
public class MessageSplitterFactory<T extends ConnectRecord<T>> {
  ...
  public List<MessageSplitter<T>> buildMessageSplitters(
      Map<String, String> config, JexlEngine jexlEngine, List<String> messageSplitterIds) {
    ...
    for (String splitterId : Optional.ofNullable(messageSplitterIds).orElse(Lists.newArrayList())) {
      Map<String, String> settings =
          MapUtils.getMapWithPrefix(config, MESSAGE_SPLITTER + splitterId + ".");
      String splitPattern = settings.get("pattern");
      Preconditions.checkNotNull(
          splitPattern, "message splitter '" + splitterId + "' splitPattern is required");
      ...
      MessageSplitter<T> requestSplitter =
          new MessageSplitter<>(
              fromStringPartToRecordFunction,
              splitterId,
              jexlEngine,
              matchingExpression,
              splitPattern,
