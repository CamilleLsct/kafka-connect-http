...
/**
 * this class is used to map a sink record to an http request using JEXL expressions. The JEXL
 * expressions are evaluated using the sink record as context. Each part of the http request can be
 * evaluated with a JEXL expression.
 */
public class JEXLHttpRequestMapper extends AbstractHttpRequestMapper {
  ...
  public JEXLHttpRequestMapper(
    ...
    jexlMethodExpression =
        methodExpression != null
            ? Optional.of(jexlEngine.createExpression(methodExpression))
            : Optional.empty();
    jexlBodyTypeExpression =
        bodyTypeExpression != null
            ? Optional.of(jexlEngine.createExpression(bodyTypeExpression))
            : Optional.empty();
    jexlBodyExpression =
        bodyExpression != null
            ? Optional.of(jexlEngine.createExpression(bodyExpression))
            : Optional.empty();
    jexlHeadersExpression =
        headersExpression != null
            ? Optional.of(jexlEngine.createExpression(headersExpression))
            : Optional.empty();
  }
  ...
  public HttpRequest map(ConnectRecord sinkRecord) {
    ...
    HttpRequest.Method method =
        jexlMethodExpression
            .map(
                jexlExpression ->
                    HttpRequest.Method.valueOf((String) jexlExpression.evaluate(context)))
            .orElse(HttpRequest.Method.GET);
    ...
    String content =
        jexlBodyExpression.isPresent()
            ? jexlBodyExpression
                .map(jexlExpression -> (String) jexlExpression.evaluate(context))
                .orElse(null)
            : null;
    ...
    Map<String, List<String>> headers =
        jexlHeadersExpression.isPresent()
            ? (Map<String, List<String>>) jexlHeadersExpression.get().evaluate(context)
            : Maps.newHashMap();
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(jexlMatchingExpression, that.jexlMatchingExpression)
        && Objects.equals(jexlUrlExpression, that.jexlUrlExpression)
        && Objects.equals(jexlMethodExpression, that.jexlMethodExpression)
        && Objects.equals(jexlBodyTypeExpression, that.jexlBodyTypeExpression)
        && Objects.equals(jexlBodyExpression, that.jexlBodyExpression)
        && Objects.equals(jexlHeadersExpression, that.jexlHeadersExpression);
  }

  @Override
  public int hashCode() {
    return Objects.hash(
        jexlMatchingExpression,
        jexlUrlExpression,
        jexlMethodExpression,
        jexlBodyTypeExpression,
        jexlBodyExpression,
