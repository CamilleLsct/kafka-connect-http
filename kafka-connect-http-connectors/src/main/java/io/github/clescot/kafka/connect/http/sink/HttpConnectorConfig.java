...
public class HttpConnectorConfig extends AbstractConfig {
  ...
  public HttpConnectorConfig(ConfigDef configDef, Map<String, String> originals) {
    ...
    this.bearerAuthCacheExpiryBufferSeconds =
        getInt(PRODUCER_BEARER_AUTH_CACHE_EXPIRY_BUFFER_SECONDS);
    ...
    // meter Registry
    this.meterRegistryExporterJmxActivate =
        Boolean.parseBoolean(getString(METER_REGISTRY_EXPORTER_JMX_ACTIVATE));
    this.meterRegistryExporterPrometheusActivate =
        Boolean.parseBoolean(getString(METER_REGISTRY_EXPORTER_PROMETHEUS_ACTIVATE));
    this.meterRegistryExporterPrometheusPort = getInt(METER_REGISTRY_EXPORTER_PROMETHEUS_PORT);
    this.meterRegistryBindMetricsExecutorService =
        Boolean.parseBoolean(getString(METER_REGISTRY_BIND_METRICS_EXECUTOR_SERVICE));
    this.meterRegistryBindMetricsJvmClassloader =
        Boolean.parseBoolean(getString(METER_REGISTRY_BIND_METRICS_JVM_CLASSLOADER));
    this.meterRegistryBindMetricsJvmProcessor =
        Boolean.parseBoolean(getString(METER_REGISTRY_BIND_METRICS_JVM_PROCESSOR));
    this.meterRegistryBindMetricsJvmGc =
        Boolean.parseBoolean(getString(METER_REGISTRY_BIND_METRICS_JVM_GC));
    this.meterRegistryBindMetricsJvmInfo =
        Boolean.parseBoolean(getString(METER_REGISTRY_BIND_METRICS_JVM_INFO));
    this.meterRegistryBindMetricsJvmMemory =
        Boolean.parseBoolean(getString(METER_REGISTRY_BIND_METRICS_JVM_MEMORY));
    this.meterRegistryBindMetricsJvmThread =
        Boolean.parseBoolean(getString(METER_REGISTRY_BIND_METRICS_JVM_THREAD));
    this.meterRegistryBindMetricsLogback =
        Boolean.parseBoolean(getString(METER_REGISTRY_BIND_METRICS_LOGBACK));
    this.meterRegistryTagIncludeLegacyHost =
        Boolean.parseBoolean(getString(METER_REGISTRY_TAG_INCLUDE_LEGACY_HOST));
    this.meterRegistryTagIncludeUrlPath =
        Boolean.parseBoolean(getString(METER_REGISTRY_TAG_INCLUDE_URL_PATH));

    this.httpClientImplementation = getString(CONFIG_HTTP_CLIENT_IMPLEMENTATION);

    this.queueName =
        Optional.ofNullable(getString(ConfigConstants.QUEUE_NAME))
            .orElse(QueueFactory.DEFAULT_QUEUE_NAME);
    this.publishMode =
        PublishMode.valueOf(
            Optional.ofNullable(getString(PUBLISH_MODE)).orElse(PublishMode.NONE.name()));
    ...
    if (QueueFactory.queueMapIsEmpty()
        && PublishMode.IN_MEMORY_QUEUE.name().equalsIgnoreCase(publishMode.name())) {
      LOGGER.warn(
          "no pre-existing queue exists. this HttpSourceConnector has created a '{}' one. It needs to consume a queue filled with a SinkConnector. Ignore this message if a SinkConnector will be created after this one.",
          queueName);
    }
    ...
    this.generateMissingRequestId =
        Boolean.parseBoolean(getString(CONFIG_GENERATE_MISSING_REQUEST_ID));
    this.generateMissingCorrelationId =
        Boolean.parseBoolean(getString(CONFIG_GENERATE_MISSING_CORRELATION_ID));
    ...
    this.maxWaitTimeRegistrationOfQueueConsumerInMs =
        getLong(WAIT_TIME_REGISTRATION_QUEUE_CONSUMER_IN_MS);
    this.pollDelayRegistrationOfQueueConsumerInMs =
        getInt(POLL_DELAY_REGISTRATION_QUEUE_CONSUMER_IN_MS);
    this.pollIntervalRegistrationOfQueueConsumerInMs =
        getInt(POLL_INTERVAL_REGISTRATION_QUEUE_CONSUMER_IN_MS);
    Optional<List<String>> staticRequestHeaderNames =
        Optional.ofNullable(getList(CONFIG_STATIC_REQUEST_HEADER_NAMES));
    ...
    for (String headerName : additionalHeaderNamesList) {
      ...
      Preconditions.checkNotNull(
          ...
          "'"
              + key
              + "' is not configured as a parameter. original parameters : \n"
              + originalStrings);
    ...
    this.defaultRequestMapperMode =
        Optional.of(
                MapperMode.valueOf(
                    getString(DEFAULT_REQUEST_MAPPER_PREFIX + REQUEST_MAPPER_DEFAULT_MODE)))
            .orElse(MapperMode.DIRECT);
    this.defaultUrlExpression =
        getString(DEFAULT_REQUEST_MAPPER_PREFIX + REQUEST_MAPPER_DEFAULT_URL_EXPRESSION);
    this.defaultMethodExpression =
        getString(DEFAULT_REQUEST_MAPPER_PREFIX + REQUEST_MAPPER_DEFAULT_METHOD_EXPRESSION);
    this.defaultBodyTypeExpression =
        Optional.ofNullable(
                getString(
                    DEFAULT_REQUEST_MAPPER_PREFIX + REQUEST_MAPPER_DEFAULT_BODYTYPE_EXPRESSION))
            .orElse("'" + BodyType.STRING + "'");
    this.defaultBodyExpression =
        getString(DEFAULT_REQUEST_MAPPER_PREFIX + REQUEST_MAPPER_DEFAULT_BODY_EXPRESSION);
    this.defaultHeadersExpression =
        getString(DEFAULT_REQUEST_MAPPER_PREFIX + REQUEST_MAPPER_DEFAULT_HEADERS_EXPRESSION);
    this.httpRequestMapperIds =
        Optional.ofNullable(getList(HTTP_REQUEST_MAPPER_IDS)).orElse(Lists.newArrayList());
    this.messageSplitterIds =
        Optional.ofNullable(getList(MESSAGE_SPLITTER_IDS)).orElse(Lists.newArrayList());
  }
  ...
  public String toString() {
    return "HttpSinkConnectorConfig{"
        ...
        + ", configurationIds="
        + configurationIds
        + ", httpRequestMapperIds="
        + httpRequestMapperIds
        + ", httpRequestSplitterIds="
        + messageSplitterIds
        + ", defaultRequestMapperMode="
        + defaultRequestMapperMode
  ...
  public boolean equals(Object o) {
    ...
    return producerSchemaRegistryCacheCapacity == that.producerSchemaRegistryCacheCapacity
        ...
        && maxWaitTimeRegistrationOfQueueConsumerInMs
            == that.maxWaitTimeRegistrationOfQueueConsumerInMs
        && pollDelayRegistrationOfQueueConsumerInMs == that.pollDelayRegistrationOfQueueConsumerInMs
        && pollIntervalRegistrationOfQueueConsumerInMs
            == that.pollIntervalRegistrationOfQueueConsumerInMs
        ...
        && Objects.equals(
            bearerAuthCacheExpiryBufferSeconds, that.bearerAuthCacheExpiryBufferSeconds)
