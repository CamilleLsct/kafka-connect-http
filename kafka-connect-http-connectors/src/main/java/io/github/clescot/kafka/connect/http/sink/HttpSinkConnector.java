...
public class HttpSinkConnector extends SinkConnector {
  ...
  public Class<? extends Task> taskClass() {
    Map<String, Object> defaultConfigurationSettings =
        httpConnectorConfig.originalsWithPrefix("config." + DEFAULT_CONFIGURATION_ID + ".");
    String httpClientImplementation =
        (String)
            Optional.ofNullable(defaultConfigurationSettings.get(CONFIG_HTTP_CLIENT_IMPLEMENTATION))
                .orElse(OKHTTP_IMPLEMENTATION);
    if (AHC_IMPLEMENTATION.equalsIgnoreCase(httpClientImplementation)) {
      ...
      LOGGER.error(
          "unknown HttpClient implementation : must be either 'ahc' or 'okhttp', but is '{}'",
  ...
  public List<Map<String, String>> taskConfigs(int taskCount) {
    ...
    Preconditions.checkArgument(
        this.httpConnectorConfig.getConfigurationIds().contains(DEFAULT_CONFIGURATION_ID));
    ...
    for (int i = 0; i < taskCount; i++) {
      ...
      // scalability with multiple connector instances is handled by partitioning the incoming
      // topic.
  ...
  public ConfigDef config() {
    HttpConfigDefinition httpConfigDefinition =
        new HttpConfigDefinition(Optional.ofNullable(settings).orElse(Maps.newHashMap()));
