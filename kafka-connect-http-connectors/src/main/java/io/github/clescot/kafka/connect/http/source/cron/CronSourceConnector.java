...
/**
 * CronSourceConnector is a Kafka Connect Source Connector that triggers HTTP requests based on a
 * cron schedule. It uses Quartz Scheduler to manage the scheduling of HTTP requests.
 */
public class CronSourceConnector extends SourceConnector {
  ...
  public List<Map<String, String>> taskConfigs(int maxTasks) {
    ...
    Preconditions.checkNotNull(
        ...
        "httpCronSourceConnectorConfig must not be null. Call start() first.");
    int numGroups = Math.min(httpCronSourceConnectorConfig.getJobs().size(), maxTasks);
    List<List<String>> partitions =
        ConnectorUtils.groupPartitions(httpCronSourceConnectorConfig.getJobs(), numGroups);
    for (List<String> partition : partitions) {
      List<String> list = partition.stream().map(jobId -> "job." + jobId).toList();
      Map<String, String> subSettings =
          MapUtils.filterEntriesStartingWithPrefixes(
              httpCronSourceConnectorConfig.originalsStrings(), list.toArray(new String[0]));
      subSettings.put("jobs", String.join(",", partition));
      subSettings.putAll(
          // Filter out job-specific settings and keep only the common settings
          httpCronSourceConnectorConfig.originalsStrings().entrySet().stream()
