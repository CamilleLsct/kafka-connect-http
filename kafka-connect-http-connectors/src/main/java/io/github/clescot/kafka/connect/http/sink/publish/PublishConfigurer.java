...
public class PublishConfigurer {
  ...
  private static final List<String> JSON_SCHEMA_VERSIONS =
      Lists.newArrayList("draft_4", "draft_6", "draft_7", "draft_2019_09");
  ...
  public void configureProducerPublishMode(
      HttpConnectorConfig httpConnectorConfig, KafkaProducer<String, Object> producer) {

    Preconditions.checkNotNull(
        httpConnectorConfig, "'httpSinkConnectorConfig' is null but required");

    // low-level producer is configured (bootstrap.servers is a requirement)
    Preconditions.checkArgument(
        !Strings.isNullOrEmpty(httpConnectorConfig.getProducerBootstrapServers()),
        "producer.bootstrap.servers is not set.\n" + httpConnectorConfig);
    Preconditions.checkArgument(
        !Strings.isNullOrEmpty(httpConnectorConfig.getProducerSuccessTopic()),
        "producer.success.topic is not set.\n" + httpConnectorConfig);
    Preconditions.checkArgument(
        !Strings.isNullOrEmpty(httpConnectorConfig.getProducerErrorTopic()),
        "producer.error.topic is not set.\n" + httpConnectorConfig);
  ...
  private void checkKafkaConnectivity(
      HttpConnectorConfig sinkConnectorConfig, KafkaProducer<String, Object> producer) {
    LOGGER.info(
        "test connectivity to kafka cluster for producer with address :'{}' for topic:'{}'",
        sinkConnectorConfig.getProducerBootstrapServers(),
        sinkConnectorConfig.getProducerSuccessTopic());
    ...
    try {
      ...
      for (Map.Entry<String, Object> entry :
          sinkConnectorConfig.originalsWithPrefix(PRODUCER_PREFIX).entrySet()) {
  ...
  private <T> Serializer<T> getSerializer(HttpConnectorConfig httpConnectorConfig) {
    ...
    if (JSON_SCHEMA.equalsIgnoreCase(format)) {
      ...
      LOGGER.info(
          "producer jsonSchemaSerdeConfigFactory: 'autoRegisterSchemas':'{}'", autoRegisterSchemas);
      ...
      Preconditions.checkArgument(
          !jsonSchemaSpecVersion.isEmpty(), "'jsonSchemaSpecVersion' must not be an empty string");
      Preconditions.checkArgument(
          JSON_SCHEMA_VERSIONS.contains(jsonSchemaSpecVersion.toLowerCase()),
          "jsonSchemaSpecVersion supported values are 'draft_4','draft_6','draft_7','draft_2019_09' but not '"
              + jsonSchemaSpecVersion
              + "'");
      serdeConfig.put(KafkaJsonSchemaSerializerConfig.SCHEMA_SPEC_VERSION, jsonSchemaSpecVersion);
      LOGGER.info(
          "producer jsonSchemaSerdeConfigFactory: 'jsonSchemaSpecVersion':'{}'",
          jsonSchemaSpecVersion);

      serdeConfig.put(WRITE_DATES_AS_ISO8601, writeDatesAsIso8601);
      LOGGER.info(
          "producer jsonSchemaSerdeConfigFactory: 'writeDatesAsIso8601':'{}'", writeDatesAsIso8601);
      ...
      LOGGER.info(
          "producer jsonSchemaSerdeConfigFactory: 'oneOfForNullables':'{}'", oneOfForNullables);
      ...
      LOGGER.info(
          "producer jsonSchemaSerdeConfigFactory: 'failInvalidSchema':'{}'", failInvalidSchema);
      ...
      LOGGER.info(
          "producer jsonSchemaSerdeConfigFactory: 'failUnknownProperties':'{}'",
          failUnknownProperties);

      serdeConfig.put(
          "key.subject.name.strategy", httpConnectorConfig.getProducerKeySubjectNameStrategy());
      serdeConfig.put(
          "value.subject.name.strategy", httpConnectorConfig.getProducerValueSubjectNameStrategy());

      SchemaRegistryClient schemaRegistryClient =
          getSchemaRegistryClient(httpConnectorConfig, schemaRegistryUrl);
  ...
  private SchemaRegistryClient getSchemaRegistryClient(
      HttpConnectorConfig httpConnectorConfig, String schemaRegistryUrl) {

    Map<String, Object> schemaRegistryClientConfig = Maps.newHashMap();
    if (httpConnectorConfig.getMissingIdCacheTTLSec() != null) {
      schemaRegistryClientConfig.put(
          MISSING_ID_CACHE_TTL_SEC, httpConnectorConfig.getMissingIdCacheTTLSec());
    }
    if (httpConnectorConfig.getMissingVersionCacheTTLSec() != null) {
      schemaRegistryClientConfig.put(
          MISSING_VERSION_CACHE_TTL_SEC, httpConnectorConfig.getMissingVersionCacheTTLSec());
    }
    if (httpConnectorConfig.getMissingSchemaCacheTTLSec() != null) {
      schemaRegistryClientConfig.put(
          MISSING_SCHEMA_CACHE_TTL_SEC, httpConnectorConfig.getMissingVersionCacheTTLSec());
    }
    ...
    if (httpConnectorConfig.getMissingCacheSize() != null) {
      schemaRegistryClientConfig.put(
          ...
          httpConnectorConfig.getBearerAuthCacheExpiryBufferSeconds());
    }
    if (httpConnectorConfig.getBearerAuthScopeClaimName() != null) {
      schemaRegistryClientConfig.put(
          BEARER_AUTH_SCOPE_CLAIM_NAME, httpConnectorConfig.getBearerAuthScopeClaimName());
    }
    if (httpConnectorConfig.getBearerAuthSubClaimName() != null) {
      schemaRegistryClientConfig.put(
          BEARER_AUTH_SUB_CLAIM_NAME, httpConnectorConfig.getBearerAuthSubClaimName());
    }
