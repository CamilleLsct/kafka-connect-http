...
/**
 * HttpSinkTask is a Kafka Connect SinkTask that processes SinkRecords, splits messages, maps them
 * to HttpRequests, and sends them to an HttpClient.
 *
...
public abstract class HttpSinkTask<C extends HttpClient<R, S>, R, S> extends SinkTask {
  public static final BiFunction<SinkRecord, String, SinkRecord>
      FROM_STRING_PART_TO_SINK_RECORD_FUNCTION =
          (sinkRecord, string) ->
              new SinkRecord(
                  sinkRecord.topic(),
  ...
  public HttpSinkTask(
      HttpClientFactory<C, R, S> httpClientFactory, KafkaProducer<String, Object> producer) {
  ...
  public void start(Map<String, String> settings) {
    ...
    try {
      ...
      if (errantRecordReporter == null) {
        LOGGER.warn(
            "Dead Letter Queue (DLQ) is not enabled. it is recommended to configure a Dead Letter Queue for a better error handling.");
      }
    } catch (NoSuchMethodError | NoClassDefFoundError e) {
      LOGGER.warn(
          "errantRecordReporter has been added to Kafka Connect since 2.6.0 release. you should upgrade the Kafka Connect Runtime shortly.");
    ...
    this.defaultHttpRequestMapper =
        httpRequestMapperFactory.buildDefaultHttpRequestMapper(
            jexlEngine,
    ...
    this.httpRequestMappers =
        httpRequestMapperFactory.buildCustomHttpRequestMappers(
            httpConnectorConfig.originalsStrings(),
            jexlEngine,
            httpConnectorConfig.getList(HTTP_REQUEST_MAPPER_IDS));

    // message splitters
    MessageSplitterFactory<SinkRecord> messageSplitterFactory =
        new MessageSplitterFactory<>(FROM_STRING_PART_TO_SINK_RECORD_FUNCTION);

    this.messageSplitters =
        messageSplitterFactory.buildMessageSplitters(
            httpConnectorConfig.originalsStrings(),
            jexlEngine,
            httpConnectorConfig.getList(MESSAGE_SPLITTER_IDS));
    httpTask =
        new HttpTask<>(
            httpClientFactory,
            httpConnectorConfig.getList(REQUEST_GROUPER_IDS),
            httpConnectorConfig.originalsStrings(),
            httpConnectorConfig.getInt(HTTP_CLIENT_ASYNC_FIXED_THREAD_POOL_SIZE),
  ...
  public @Nullable List<HttpExchange> putAndGetExchanges(Collection<SinkRecord> records) {
    ...
    Preconditions.checkNotNull(
        httpTask, "httpTask is null. 'start' method must be called once before put");
    List<Pair<SinkRecord, HttpRequest>> preparedRequests = prepareRequests(records);
    // List<SinkRecord>-> SinkRecord
    List<CompletableFuture<HttpExchange>> completableFutures =
        preparedRequests.stream().map(this::callAndPublish).toList();
  ...
  private static JexlEngine buildJexlEngine() {
    ...
    JexlPermissions permissions =
        new JexlPermissions.ClassPermissions(
            SinkRecord.class, ConnectRecord.class, HttpRequest.class);
    // Create the engine
  ...
  public CompletableFuture<HttpExchange> callAndPublish(Pair<SinkRecord, HttpRequest> pair) {
    ...
    if (httpTask.isClosed(httpRequest)) {
      ...
      publishToDeadLetterQueue(
          this.httpConnectorConfig.getProducerDlqTopic(), httpRequest, openedUntil);
      ...
      HttpExchange httpExchange =
          httpTask
              ...
              .buildExchange(
                  httpRequest,
                  new HttpResponse(
                      HttpClient.SERVER_ERROR_STATUS_CODE,
                      "configuration is disabled until " + openedUntil),
  ...
  private void publishToDeadLetterQueue(
      String dlqTopic, HttpRequest httpRequest, String openedUntil) {
    LOGGER.error(
        "HttpRequest sent to dead letter queue because the task is opened (i.e disabled) until '{}': '{}'",
        openedUntil,
        httpRequest);
    List<Header> headers =
        Lists.newArrayList(new RecordHeader("openedUntil", openedUntil.getBytes()));
    ProducerRecord<String, Object> myRecord =
        new ProducerRecord<>(
            dlqTopic, null, System.currentTimeMillis(), null, httpRequest, headers);
    this.producer.send(myRecord);
  }

  private List<SinkRecord> splitMessage(SinkRecord sinkRecord) {
    Optional<MessageSplitter<SinkRecord>> splitterFound =
        messageSplitters.stream()
            .filter(messageSplitter -> messageSplitter.matches(sinkRecord))
            .findFirst();
    // splitter
  ...
  public List<Pair<SinkRecord, HttpRequest>> prepareRequests(Collection<SinkRecord> records) {
    ...
    List<Pair<SinkRecord, HttpRequest>> requests =
        stream
  ...
  private void publishInProducerMode(
      HttpExchange httpExchange,
      String producerContent,
      String producerSuccessTopic,
      String producerErrorTopic) {
    LOGGER.debug(
        "publish.mode : 'PRODUCER' : HttpExchange success will be published at topic : '{}'",
        producerSuccessTopic);
    LOGGER.debug(
        "publish.mode : 'PRODUCER' : HttpExchange error will be published at topic : '{}'",
        producerErrorTopic);
    String targetTopic = httpExchange.isSuccess() ? producerSuccessTopic : producerErrorTopic;
    ProducerRecord<String, Object> myRecord =
        mapToRecord(httpExchange, producerContent, targetTopic);
    ...
    LOGGER.debug(
        "✉✉ record sent ✉✉ : topic:{},partition:{},offset:{},timestamp:{}",
        topic,
        partition,
        offset,
        timestamp);
  ...
  @NotNull
  private ProducerRecord<String, Object> mapToRecord(
      HttpExchange httpExchange, String producerContent, String targetTopic) {
    ProducerRecord<String, Object> myRecord;
  ...
  public Function<HttpExchange, HttpExchange> publish() {
    return httpExchange -> {
      ...
      if (PublishMode.IN_MEMORY_QUEUE.equals(publishMode)) {
        ...
        publishInProducerMode(
            httpExchange,
            this.httpConnectorConfig.getProducerContent(),
  ...
  private void publishInInMemoryQueueMode(HttpExchange httpExchange, String queueName) {
    LOGGER.debug(
        "publish.mode : 'IN_MEMORY_QUEUE': http exchange published to queue '{}':{}",
        queueName,
        httpExchange);
    boolean offer = queue.offer(mapToRecord(httpExchange));
    if (!offer) {
      LOGGER.error("sinkRecord  not added to the 'in memory' queue:{}", queueName);
    }
  }
