...
public class HttpRequestMapperFactory {
  ...
  public HttpRequestMapper buildDefaultHttpRequestMapper(
    ...
    switch (defaultRequestMapperMode) {
      case JEXL:
        {
          Preconditions.checkNotNull(
              ...
              "'" + REQUEST_MAPPER_DEFAULT_URL_EXPRESSION + "' need to be set");
          httpRequestMapper =
              new JEXLHttpRequestMapper(
                  DEFAULT,
                  jexlEngine,
                  JEXL_ALWAYS_MATCHES,
                  defaultUrlExpression,
      ...
      default:
        {
          httpRequestMapper = new DirectHttpRequestMapper(DEFAULT, jexlEngine, JEXL_ALWAYS_MATCHES);
          break;
        }
    }
  ...
  public List<HttpRequestMapper> buildCustomHttpRequestMappers(
      Map<String, String> config, JexlEngine jexlEngine, List<String> requestMapperIds) {
    List<HttpRequestMapper> requestMappers = Lists.newArrayList();
    for (String httpRequestMapperId :
        Optional.ofNullable(requestMapperIds).orElse(Lists.newArrayList())) {
      HttpRequestMapper httpRequestMapper;
      ...
      MapperMode mapperMode =
          MapperMode.valueOf(
              Optional.ofNullable(settings.get(modeKey))
                  .orElse(MapperMode.DIRECT.name())
                  .toString());
      switch (mapperMode) {
        case JEXL:
          {
        ...
        default:
          {
            httpRequestMapper =
                new DirectHttpRequestMapper(
                    httpRequestMapperId, jexlEngine, settings.get("matcher"));
            break;
          }
      }
