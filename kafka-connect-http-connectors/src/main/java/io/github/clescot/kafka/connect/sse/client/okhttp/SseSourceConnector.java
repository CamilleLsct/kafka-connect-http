...
public class SseSourceConnector extends SourceConnector {
  ...
  public List<Map<String, String>> taskConfigs(int maxTasks) {
    ...
    Preconditions.checkNotNull(
        this.sseConnectorConfig, "sseConnectorConfig must not be null. Call start() first.");
    int numGroups = Math.min(this.sseConnectorConfig.getConfigurationIds().size(), maxTasks);
    List<List<String>> partitions =
        ConnectorUtils.groupPartitions(this.sseConnectorConfig.getConfigurationIds(), numGroups);
    List<Map<String, String>> configs = new ArrayList<>(maxTasks);
    for (List<String> partition : partitions) {
      List<String> list =
          partition.stream().map(configurationId -> "config." + configurationId).toList();
      Map<String, String> subSettings =
          MapUtils.filterEntriesStartingWithPrefixes(
              this.sseConnectorConfig.originalsStrings(), list.toArray(new String[0]));
