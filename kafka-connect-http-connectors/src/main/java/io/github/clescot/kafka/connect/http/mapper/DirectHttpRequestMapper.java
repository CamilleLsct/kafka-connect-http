...
public class DirectHttpRequestMapper extends AbstractHttpRequestMapper {
  ...
  public HttpRequest map(ConnectRecord sinkRecord) {
    ...
    if (Struct.class.isAssignableFrom(valueClass)) {
      ...
      LOGGER.warn(
          "value is an instance of the class '{}' not handled by the WsSinkTask",
          valueClass.getName());
      throw new ConnectException(
          "value is an instance of the class "
              + valueClass.getName()
              + " not handled by the WsSinkTask");
    }
  ...
  public String toString() {
    return "DirectHttpRequestMapper{" + "expression=" + expression + ", id='" + id + '\'' + '}';
  }
}
