...
public class HttpRequestAdapter {
  ...
  public static final Schema SCHEMA =
      SchemaBuilder.struct()
          ...
          .field(
              HEADERS_FIELD,
              SchemaBuilder.map(
                      Schema.STRING_SCHEMA, SchemaBuilder.array(Schema.STRING_SCHEMA).schema())
                  .build())
          ...
          .field(
              BODY_AS_FORM_FIELD,
              SchemaBuilder.map(Schema.STRING_SCHEMA, Schema.STRING_SCHEMA).optional().schema())
          .field(BODY_AS_STRING_FIELD, Schema.OPTIONAL_STRING_SCHEMA)
          .field(
              PARTS_FIELD,
              SchemaBuilder.map(Schema.STRING_SCHEMA, HttpPartAdapter.SCHEMA).optional().schema())
          .field(
              ATTRIBUTES_FIELD,
              SchemaBuilder.map(Schema.STRING_SCHEMA, Schema.STRING_SCHEMA).optional().schema())
          .schema();
  ...
  public HttpRequestAdapter(Struct requestAsstruct) {
    ...
    HttpRequest.Method method =
        HttpRequest.Method.valueOf(requestAsstruct.getString(METHOD_FIELD).toUpperCase());
    Preconditions.checkNotNull(method, "'method' is required");

    BodyType bodyType =
        BodyType.valueOf(
            Optional.ofNullable(requestAsstruct.getString(BODY_TYPE_FIELD))
                .orElse(BodyType.STRING.name()));

    String bodyAsByteArrayField = requestAsstruct.getString(BODY_AS_BYTE_ARRAY_FIELD);
    byte[] bodyAsByteArray =
        bodyAsByteArrayField != null ? bodyAsByteArrayField.getBytes(StandardCharsets.UTF_8) : null;
    ...
    if (structs != null) {
      ...
      for (Map.Entry<String, Struct> entry : structs.entrySet()) {
        ...
        if (!headersFromPartAreValid(httpPart)) {
          LOGGER.warn(
              "this is a multipart request. headers from part are not valid : there is at least one header that is not 'Content-Disposition', 'Content-Type' or 'Content-Transfer-Encoding'. clearing headers from this part");
  ...
  public Struct toStruct() {
    return new Struct(SCHEMA)
        ...
        .put(
            ...
            httpRequest.getBodyAsByteArray() != null
                ? new String(httpRequest.getBodyAsByteArray(), StandardCharsets.UTF_8)
                : null)
