...
import io.github.clescot.client.http.proxy.ProxySelectorFactory;
...
public interface HttpClientFactory<C extends HttpClient<R, S>, R, S> {
    ...
    static <C extends HttpClient<R, S>, R, S>Map<String,C> buildConfigurations(
        ...
        for (String configId : configurationIds) {
            Map<String, String> config = Maps.newHashMap(HttpClientConfigDefinition.getMapWithPrefix(originals, "config." + configId + "."));
