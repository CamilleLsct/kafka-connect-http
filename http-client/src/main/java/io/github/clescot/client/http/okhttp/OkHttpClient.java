...
import io.github.clescot.client.http.AbstractHttpClient;
import io.github.clescot.client.http.HttpClient;
import io.github.clescot.client.http.TimingData;
...
import static io.github.clescot.core.http.MediaType.APPLICATION_OCTET_STREAM;
...
public class OkHttpClient extends AbstractHttpClient<Request, Response> {
    ...
    private RequestBody getMultiPartRequestBody(HttpRequest httpRequest, String firstContentType) {
        ...
        multipartBuilder.setType(MediaType.parse(io.github.clescot.core.http.MediaType.MULTIPART_FORM_DATA));
    ...
    private Map<String, List<String>> filterHeaders(Map<String, List<String>> headers) {
        return headers != null ? headers.entrySet().stream()
                .filter(entry -> !entry.getKey().equalsIgnoreCase(io.github.clescot.core.http.MediaType.KEY))
                .collect(
                        Collectors.toMap(
                                Map.Entry::getKey,
                                Map.Entry::getValue
    ...
    public HttpResponse buildResponse(Response response) {
        ...
        try {
            ...
            String contentType = response.header(io.github.clescot.core.http.MediaType.KEY);
            if (contentType != null && !contentType.isEmpty()) {
                httpResponse.setContentType(contentType);
            } else {
                //default content type
                httpResponse.setContentType(io.github.clescot.core.http.MediaType.APPLICATION_JSON);
            }
